...
```bash
npx hardhat run scripts/egp-tbd/testCreateAndExecute.ts --no-compile --network hardhat
```
...
```bash
npx hardhat run scripts/egp-tbd/executables/executableCreateProposal.ts --no-compile --network mainnet
```
...
```bash
 npx hardhat run scripts/egp-tbd/executables/executableExecuteProposal.ts --no-compile --network mainnet
```
...
```bash
 npx hardhat run scripts/egp-tbd/executables/executableExecuteTimelock.ts --no-compile --network mainnet
```